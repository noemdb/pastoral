        ...
        @if ($curriculum_id)
            <div class="grid grid-cols-1 gap-4 mt-2">
                <div class="rounded bg-white shadow border p-1  {{ ($step==4) ? 'opacity-100' : 'opacity-30'}}">
                    ...
                    <div class="mb-4">
                        @php $name = 'lapse_id'; $model = ''.$name; $key = Str::random(10); @endphp
                        <x-jet-label for="{{$model}}" value="{{ $list_comment_inscription[$name] ?? null }}" />
                        <x-select wire:key="{{$key}}" wire:model="{{$model}}" :options="$lapses_list" id="{{$model}}" class=" w-full " />
                        @error($model) <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
            ...
            @if ($lapse_id)
                ...
                <div class="grid grid-cols-1 gap-4 mt-2">
                    <div class="rounded bg-white shadow border p-1  {{ ($step==5) ? 'opacity-100' : 'opacity-30'}}">
                ...
                @if ($level_id)
                    <div class="grid grid-cols-1 gap-4 mt-2 {{ ($step==6) ? '!opacity-100' : '!opacity-30'}}">
                        <div class="rounded bg-white shadow border p-1">
                            <h3 class="block font-semibold text-lg text-gray-700 dark:text-gray-100">Paso 6</h3>
                            <div class="mb-4">
