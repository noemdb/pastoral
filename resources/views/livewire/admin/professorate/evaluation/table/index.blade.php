{{-- 'pevaluation_id','objetivo','description','observations','objetivo','header','body','footer','attachment','status', --}}
@php
...
<div class="overflow-x-auto relative shadow-md sm:rounded-lg">
    ...
    <div class="mb-4 flex justify-between">
        <div class="w-3/4">
            @php $name = 'search'; $model = 'evaluation.'.$name; @endphp
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        @forelse($evaluations as $evaluation)
            ...
            @php 
                $pevaluation = $evaluation->pevaluation;
            ...
            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700 {{($evaluation->id == $pevaluation_id) ? 'bg-gray-200' : null}}">
                ...
                <td class="{{ $class['description'] ?? ''}}">{{$evaluation->description ?? ''}}</td>

                <td class="{{ $class['color'] ?? ''}}">
                    <div class="h-10 w-10 rounded" style="background-color: {{$evaluation->color ?? ''}}"> </div>
                </td>
                ...
                <td class="{{ $class['action'] ?? '' }}">
                    ...
                    <div class="flex items-center justify-center justify-between mb-3 shadow">
                        <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                            <x-elements.form.button-edit wire:key="evaluation-edit-{{$evaluation->id}}" wire:click="edit({{ $evaluation->id }})" >
                                <x-icon-pen class="w-4 h-4 mr-0.5" />
                            </x-elements.form.button-edit>
                            <x-elements.form.button-del wire:key="evaluation-delete-{{$evaluation->id}}" wire:click="delete({{ $evaluation->id }})" >
